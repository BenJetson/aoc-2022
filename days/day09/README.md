...
Then, by following a hypothetical _series of motions_ (your puzzle input) for
...
the head ( `H`) and tail ( `T`) must _always be touching_ (diagonally adjacent
...
example, the tail never moves, and so it only visits `1` position. However, _be
careful_: more types of motion are possible than before, so you might want to
